...
class VectorIndex(BaseIndex):
    ...
    async def _update_index(self, datas: list[dict[str:Any]], meta_data: dict):
        documents = [
            Document(
                doc_id=mdhash_id(data["content"]),
                text=data["content"],
                metadata={key: data[key] for key in meta_data.keys()},
                excluded_embed_metadata_keys=meta_data.keys(),
            )
            for data in datas
        ]
