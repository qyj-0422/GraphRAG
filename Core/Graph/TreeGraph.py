...
from Core.Config2 import Config, TreeGraphConfig
...
class TreeGraph(BaseGraph):
    ...
    def __init__(self, config, llm, encoder):
        ...
        self.tree_config = TreeGraphConfig() # Tree config
    ...
    async def _create_node(self, layer: int, text: str, children_indices: Set[int] = None):
        ...
        logger.info("Create node_id = {node_id}, children = {children}".format(node_id=node_id, children=children_indices))
        return self._graph.upsert_node(node_id = node_id, node_data = {"layer": layer, "text": text, "children": children_indices, "embedding": embedding})
    ...
    async def _extract_cluster_relationship(self, layer: int, cluster: List[TreeNode]) -> TreeNode:
        ...
        summarized_text = await self._summarize_from_cluster(cluster, self.tree_config.summarization_length)
    ...
    async def _build_tree_from_leaves(self):
        for layer in range(self.tree_config.num_layers): # build a new layer
            if len(self._graph.get_layer(layer)) <= self.tree_config.reduction_dimension + 1:
                break
